...
class KochFractal {
  ...
  ArrayList iterate(ArrayList<KochLine> before) {
    ...
    for(KochLine l : before) {
      // Now we simply create random lines
      ...
      PVector d = new PVector(random(width), random(height));
      PVector e = new PVector(random(width), random(height));
      // And we add random lines to the ArrayList
